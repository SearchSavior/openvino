"""End-to-end genai bench of v1 / v2 / v3 custom-op fusions.

Unlike bench_v2 / bench_v3 (which drive the raw ov.Core() infer-request loop
and need the serialize/reload trick so the C++ .so wins evaluate over the
Python Op subclass), this script goes through the high-level
`ov_genai.VLMPipeline(fused_dir, "CPU", extensions=[so])` API.

genai's internal Core only ever sees the .so — there is no Python `Op`
subclass registered in its namespace — so the C++ GatedDeltaRule{,V2,V3}
implementation wins evaluate natively. The Python rewrites are used here only
to *construct + serialize* the fused IR into a model dir; once it's on disk,
genai reads it back and the extension factory resolves the custom op.
...
For each version we:
  ...
  3. generate from a fixed long prompt and read genai's PerfMetrics:
       - TTFT  (time to first token ~= prefill)
       - throughput (generation tok/s)
       - per-version load time.

This is the apples-to-apples "would a genai user actually see this?" number,
as opposed to bench_v3's controlled pp512/tg32 micro-bench.

Run:
    cd study/qwen3
    QWEN3_USE_C=1 python3 scripts/working/bench_genai.py
"""
import sys
import time
...
# ~512-token prompt: a chunk of repeated instruction text, big enough that the
# prefill cost is the dominant TTFT term.
PROMPT = ("You are a careful assistant. Summarize the following note and then "
          "answer the question at the end in one short paragraph. " * 24
...
def bench(version, max_new_tokens=32):
    ...
    # warmup (also primes any lazy compile / weight-prepack)
    vlm.generate(PROMPT, generation_config=cfg)
    ...
    print(f"\n=== v{version} (genai VLMPipeline + extensions=[.so]) ===")
    print(f"  load:            {load_s:6.2f}s")
    print(f"  input tokens:    {n_in}")
    print(f"  generated:       {n_gen}")
    print(f"  TTFT (prefill):  {ttft:6.3f}s  ({n_in/ttft:6.1f} tok/s)")
...
def main():
    ...
    rows = [bench(v) for v in (1, 2, 3)]

    print(f"\n{'='*78}\nSUMMARY (genai VLMPipeline, greedy, threads=4)\n{'='*78}")
