...
// Gemma4 text-only: same BitwiseAnd sliding window chain as Gemma3/gpt-oss, but q_idx is
// Add(Range(0, seq_len), past_len) instead of Gemma3's Range(past_len, past_len+seq_len), and there
// is no token_type_ids input (that's a VLM-only feature). sliding_window = 512.
TEST_F(SDPAToPATest, SDPAToPA_Gemma4_TextOnly_SlidingWindow) {
    {
        auto beam_idx = make_param(PartialShape{DYN}, element::i32, "beam_idx");
        auto position_ids = make_param(PartialShape{DYN, DYN}, element::i64, "position_ids");
        auto attention_mask = make_param(PartialShape{DYN, DYN}, element::i64, "attention_mask");
        auto input_ids = make_param(PartialShape{DYN, DYN}, element::i64, "input_ids");
        // Gemma4 text-only has no token_type_ids (VLM-only feature)
        auto params = nodes_to_params({beam_idx, position_ids, attention_mask, input_ids});
        ...
        // Gemma4 q_idx: Range(0, seq_len) + past_len (vs Gemma3's Range(past_len, past_len+seq_len))
        auto Range_q = makeOP<v4::Range>({0, Gather_cur, 1}, {{"output_type", "f32"}});
        auto Unsqueeze_q0 = makeOP<v0::Unsqueeze>({Range_q, 0});
        auto Unsqueeze_q1 = makeOP<v0::Unsqueeze>({Unsqueeze_q0, 1});
        auto Unsqueeze_q2 = makeOP<v0::Unsqueeze>({Unsqueeze_q1, 3});
        auto Gather_past_f32 = makeOP<v0::Convert>({Gather_past}, {{"destination_type", "f32"}});
        auto q_idx = makeOP<v1::Add>({Unsqueeze_q2, Gather_past_f32}, {{"auto_broadcast", "numpy"}});

        // Gemma4 sliding_window = 512, so the mask offset is -512.0f
        auto sw_offset = makeConst(element::f32, ov::Shape({1, 1, 1, 1}), {-512.0f});
        auto sw_add = makeOP<v1::Add>({q_idx, sw_offset}, {{"auto_broadcast", "numpy"}});
        auto sw_greater = makeOP<v1::Greater>({kv_idx, sw_add}, {{"auto_broadcast", "numpy"}});
...

    {
        ...
        auto input_ids = make_param(PartialShape{DYN}, element::i64, "input_ids");
        auto position_ids = make_param(PartialShape{DYN}, element::i64, "position_ids");
        // No token_type_ids parameter for text-only Gemma4
        ...
        // sliding_window = 512: Squeeze/Convert/Multiply the -512.0f constant
        auto sw_neg = makeConst(element::f32, ov::Shape({1, 1, 1, 1}), {-512.0f});
        auto Squeeze_sw = makeOP<v15::Squeeze>({sw_neg}, {{"allow_axis_skip", false}});
        auto Convert_sw = makeOP<v0::Convert>({Squeeze_sw}, {{"destination_type", "i32"}});
        auto sliding_window = makeOP<v1::Multiply>({Convert_sw, -1}, {{"auto_broadcast", "numpy"}});
        ...
        // No token_type_ids for text-only Gemma4: PA arg[25] is a zero-sized constant placeholder
        auto token_type_ids_placeholder = v0::Constant::create(element::i32, Shape{0}, {});
        auto qq_bias = v0::Constant::create(element::u8, Shape{0}, {});
        auto qq_bias_begins = v0::Constant::create(element::i32, Shape{0}, {});
        auto PA = std::make_shared<ov::op::PagedAttentionExtension>(
            OutputVector{Q_flat,
